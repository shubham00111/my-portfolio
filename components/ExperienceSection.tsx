...
const WorkCard = ({ position }: { position: Position }) => (
  <div className="nb-card p-6">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3 mb-5">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-3 mb-1.5 flex-wrap">
          ...
          {position.isCurrent && (
            <span className="border-3 border-foreground bg-[#ff90e8] px-2 py-0.5 text-xs font-black uppercase shadow-[2px_2px_0px_0px_#000] shrink-0">
              Current
            </span>
          )}
        </div>
    ...
    {/* Award */}
    {position.award && (
      <div className="flex items-start gap-2.5 mb-5 border-3 border-foreground bg-[#ffde59] p-3 shadow-[3px_3px_0px_0px_#000]">
        <Award size={16} className="shrink-0 mt-0.5" />
        <p className="text-sm font-semibold">{position.award}</p>
      </div>
    )}
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-0.5">
          ...
          {position.grade && (
            <span className="border-3 border-foreground bg-[#99c7fb] px-2 py-0.5 text-xs font-black shadow-[2px_2px_0px_0px_#000]">
              {position.grade}
            </span>
          )}
        </div>
